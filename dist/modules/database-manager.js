...
function createDatabaseManager(config) {
    ...
    function detectDatabaseType(image) {
        for (const [type, patterns] of Object.entries(DB_IMAGE_PATTERNS)) {
    ...
    function isMysqlType(type) {
        return type === 'mysql' || type === 'mariadb';
    }
    function getContainerEnvVar(env, key) {
        if (!env)
            return undefined;
        const prefix = `${key}=`;
     ...
     */
    async function getConnectionInfo(server) {
        const container = docker.getContainer(server.containerId);
        const inspect = await container.inspect();
        const env = inspect.Config.Env;
        if (isMysqlType(server.type)) {
            const password = getContainerEnvVar(env, 'MYSQL_ROOT_PASSWORD') ??
                getContainerEnvVar(env, 'MARIADB_ROOT_PASSWORD') ??
                '';
            const user = getContainerEnvVar(env, 'MYSQL_USER') ??
            ...
            // PostgreSQL
            const password = getContainerEnvVar(env, 'POSTGRES_PASSWORD') ?? '';
            const user = getContainerEnvVar(env, 'POSTGRES_USER') ?? 'postgres';
            return {
                host: server.dockerNetworkIp ?? server.containerName,
                port: server.port,
                user,
    ...
    async function mysqlListDatabases(server) {
        ...
        for (const line of lines) {
            ...
            if (parts.length >= 1) {
                ...
                if (['information_schema', 'performance_schema', 'sys'].includes(name))
                    continue;
                const sizeBytes = parts[1] ? parseInt(parts[1], 10) : undefined;
                databases.push({ name, sizeBytes: isNaN(sizeBytes) ? undefined : sizeBytes });
            }
        }
        return databases;
    }
    async function mysqlCreateDatabase(server, name) {
        const conn = await getConnectionInfo(server);
    ...
    async function mysqlExecuteQuery(server, db, query) {
        const conn = await getConnectionInfo(server);
        ...
        const rows = cappedLines.map((line) => {
    ...
    async function mysqlManageUser(server, user) {
        const conn = await getConnectionInfo(server);
    ...
    async function mysqlExportDatabase(server, db) {
        const conn = await getConnectionInfo(server);
    ...
    async function mysqlImportDatabase(server, db, filePath) {
        const conn = await getConnectionInfo(server);
    ...
    async function pgListDatabases(server) {
        const conn = await getConnectionInfo(server);
        const cmd = [
            'psql',
            '-U', conn.user,
    ...
    async function pgCreateDatabase(server, name) {
        const conn = await getConnectionInfo(server);
        // Validate database name (prevent injection)
        if (!/^[a-zA-Z0-9_]+$/.test(name)) {
            throw new Error('Invalid database name. Only alphanumeric characters and underscores are allowed.');
    ...
    async function pgExecuteQuery(server, db, query) {
        const conn = await getConnectionInfo(server);
        const startTime = Date.now();
    ...
    async function pgManageUser(server, user) {
        const conn = await getConnectionInfo(server);
        let sql;
        // Validate username
        if (!/^[a-zA-Z0-9_]+$/.test(user.username)) {
    ...
    async function pgExportDatabase(server, db) {
        const conn = await getConnectionInfo(server);
        const exportId = (0, uuid_1.v4)();
        const exportFile = path.join(exportDir, `${db}_${exportId}.sql`);
        // Ensure export directory exists
    ...
    async function pgImportDatabase(server, db, filePath) {
        const conn = await getConnectionInfo(server);
        const jobId = (0, uuid_1.v4)();
        // Validate file size
        const stats = fs.statSync(filePath);
    ...
    return {
         ...
         */
        async executeQuery(serverId, db, query) {
            const server = serverCache.get(serverId);
            if (!server) {
                throw new Error(`Database server not found: ${serverId}. Run discoverServers() first.`);
            }
         ...
         */
        async exportDatabase(serverId, db) {
            const server = serverCache.get(serverId);
            if (!server) {
                throw new Error(`Database server not found: ${serverId}. Run discoverServers() first.`);
            }
         ...
         */
        async importDatabase(serverId, db, file) {
            const server = serverCache.get(serverId);
            if (!server) {
                throw new Error(`Database server not found: ${serverId}. Run discoverServers() first.`);
            }
         ...
         */
        async manageUser(serverId, user) {
            const server = serverCache.get(serverId);
            if (!server) {
                throw new Error(`Database server not found: ${serverId}. Run discoverServers() first.`);
            }
