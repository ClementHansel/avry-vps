...
exports.describeExpression = describeExpression;
exports.syncCrontab = syncCrontab;
...
// ─── Implementation ────────────────────────────────────────────────────────────
function createCronManager(db, config) {
  ...
  `);
    ...
    async function listJobs() {
        ...
        return rows.map((row) => {
            const lastExec = getLastExecutionStmt.get(row.id);
    ...
    async function updateJob(id, input) {
        ...
        const row = getJobStmt.get(id);
        const lastExec = getLastExecutionStmt.get(id);
        return rowToJob(row, lastExec);
    }
    // ─── deleteJob ─────────────────────────────────────────────────────────────
    ...
    // ─── getJobHistory ─────────────────────────────────────────────────────────
    async function getJobHistory(id, limit = 50) {
        const rows = getHistoryStmt.all(id, limit);
        return rows.map(rowToExecution);
    }
    // ─── recordExecution ───────────────────────────────────────────────────────
    async function recordExecution(jobId, exitCode, output) {
        const id = (0, uuid_1.v4)();
...
function validateExpression(expr) {
    ...
    try {
        const interval = cron_parser_1.default.parseExpression(expr);
 ...
 */
async function syncCrontab(db, user, execCommand) {
    // Get all enabled jobs for this user
    const jobs = db.prepare(`SELECT * FROM cron_jobs WHERE user = ? AND enabled = 1 ORDER BY created_at ASC`).all(user);
...
function rowToJob(row, lastExec) {
    return {
        id: row.id,
