...
exports.createResourceWidget = createResourceWidget;
/**
...
// ─── Implementation ────────────────────────────────────────────────────────────
function createResourceWidget(config) {
    ...
    let procAvailableCache = null;
    // ─── /proc Availability Check ──────────────────────────────────────────
    async function isProcAvailable() {
        if (procAvailableCache !== null)
            return procAvailableCache;
        try {
            await (0, promises_2.access)(`${procPath}/stat`, promises_2.constants.R_OK);
            await (0, promises_2.access)(`${procPath}/meminfo`, promises_2.constants.R_OK);
    ...
    async function readProcStat() {
        const content = await (0, promises_1.readFile)(`${procPath}/stat`, 'utf-8');
        const cpuLine = content.split('\n').find((line) => line.startsWith('cpu '));
        if (!cpuLine)
            throw new Error('Could not parse /proc/stat');
    ...
    async function readProcMeminfo() {
        const content = await (0, promises_1.readFile)(`${procPath}/meminfo`, 'utf-8');
    ...
    async function readProcNetDev() {
        const content = await (0, promises_1.readFile)(`${procPath}/net/dev`, 'utf-8');
        const lines = content.split('\n');
        let totalRx = 0;
        let totalTx = 0;
        for (const line of lines) {
    ...
    async function getSystemMetricsFromProc() {
        // CPU
    ...
    async function getSystemMetricsFallback() {
    ...
    // ─── Collect Warnings ───────────────────────────────────────────────────
    function collectWarnings(system, containers) {
        const warnings = [];
    ...
    // ─── Monitoring Loop ────────────────────────────────────────────────────
    async function collectAndEmit(io) {
        try {
