...
export interface BannedIP {
  ip: string;
  jail: string;
  banTime: number; // seconds remaining
  reason?: string;
}
...
export function createSecurityManager(
  ...
  const insertRule = db.prepare(
    `INSERT INTO firewall_rules (id, port, protocol, source, action, description) VALUES (?, ?, ?, ?, ?, ?)`
  );

  const updateRule = db.prepare(
    `UPDATE firewall_rules SET port = ?, protocol = ?, source = ?, action = ?, description = ? WHERE id = ?`
  );
  ...
  const insertScan = db.prepare(
    `INSERT INTO security_scans (id, timestamp, score, finding_count, findings) VALUES (?, ?, ?, ?, ?)`
  );

  const getScanHistoryStmt = db.prepare(
    `SELECT * FROM security_scans WHERE timestamp >= datetime('now', ?) ORDER BY timestamp DESC`
  );

  const getLatestScan = db.prepare(
    `SELECT * FROM security_scans ORDER BY timestamp DESC LIMIT 1`
  );

  const pruneOldScans = db.prepare(
    `DELETE FROM security_scans WHERE timestamp < datetime('now', ?)`
  );

  // ─── Security Score ─────────────────────────────────────────────────────

  async function getSecurityScore(): Promise<SecurityScore> {
    ...
    const latestScan = getLatestScan.get() as SecurityScanRow | undefined;
  ...
  function computeFirewallScore(): number {
    const rules = getAllRules.all() as FirewallRuleRow[];
    ...
    const hasDenyAll = rules.some(
      (r) => r.action === 'deny' && r.source === '0.0.0.0/0'
    );
    if (hasDenyAll) score += 20;
    ...
    if (denyCount >= allowCount) score += 10;

    // Penalty for allowing all sources on sensitive ports
    const sensitiveAllowAll = rules.filter(
      (r) => r.action === 'allow' && r.source === '0.0.0.0/0' &&
  ...
  function computeIPSScore(): number {
    try {
      ...
      if (jailCount >= 3) score += 20;
      else if (jailCount >= 1) score += 10;

      // Bonus if sshd jail is active
      if (status.includes('sshd')) score += 10;
  ...
  function computeScanScore(): number {
    const latestScan = getLatestScan.get() as SecurityScanRow | undefined;

    if (!latestScan) return 50; // No scan run yet, neutral score
  ...
  async function listFirewallRules(): Promise<FirewallRule[]> {
    const rows = getAllRules.all() as FirewallRuleRow[];
    return rows.map(rowToFirewallRule);
  }

  async function addFirewallRule(rule: FirewallRuleInput): Promise<FirewallRule> {
    const fullRule: FirewallRuleInput = {
      port: rule.port,
    ...
    return {
      id,
      port: fullRule.port,
      protocol: fullRule.protocol! as FirewallProtocol,
      source: fullRule.source!,
      action: fullRule.action,
  ...
  async function editFirewallRule(id: string, rule: Partial<FirewallRuleInput>): Promise<FirewallRule> {
    const existing = getRuleById.get(id) as FirewallRuleRow | undefined;
    if (!existing) {
      throw new Error(`Firewall rule not found: ${id}`);
    }

    const updatedRule: FirewallRuleInput = {
      port: rule.port ?? existing.port,
      protocol: (rule.protocol ?? existing.protocol) as FirewallProtocol,
      source: rule.source ?? existing.source,
      action: (rule.action ?? existing.action) as FirewallAction,
    ...
    const result: FirewallRule = {
      ...
      protocol: updatedRule.protocol! as FirewallProtocol,
  ...
  function validateFirewallRule(rule: FirewallRuleInput): RuleValidation {
    return validateFirewallRuleExcluding(rule, undefined);
  }

  function validateFirewallRuleExcluding(rule: FirewallRuleInput, excludeId?: string): RuleValidation {
    const errors: string[] = [];
    ...
    if (action === 'deny') {
      ...
      if (port === panelPort && (source === '0.0.0.0/0' || ipMatchesAnyAdmin(source))) {
        errors.push(
          `Rule would block access to the VPS Panel port (${panelPort}). ` +
      ...
      for (const adminIP of adminIPs) {
        if (ipMatchesCIDR(adminIP, source)) {
          errors.push(
            `Rule would block traffic from admin IP ${adminIP}. ` +
            `This would lock out the administrator.`
          );
        }
      }
    ...
    const existingRules = getAllRules.all() as FirewallRuleRow[];
    for (const existing of existingRules) {
      ...
      if (samePort && sameProtocol && sourcesOverlap) {
        if (existing.action === action) {
          // Duplicate rule
          warnings.push(
            `Duplicate rule detected: existing rule ${existing.id} already ` +
            `${existing.action}s ${existing.protocol}/${existing.port} from ${existing.source}`
          );
        } else {
          // Contradicting rule
          warnings.push(
            `Contradicting rule detected: existing rule ${existing.id} ` +
            `${existing.action}s ${existing.protocol}/${existing.port} from ${existing.source}, ` +
  ...
  function applyFirewallRule(rule: FirewallRule): void {
    try {
      if (useUfw) {
        applyUfwRule(rule);
      } else {
  ...
  function applyUfwRule(rule: FirewallRule): void {
  ...
  function applyIptablesRule(rule: FirewallRule): void {
  ...
  function removeFirewallRuleFromSystem(rule: FirewallRule): void {
    try {
      if (useUfw) {
        removeUfwRule(rule);
      } else {
  ...
  function removeUfwRule(rule: FirewallRule): void {
    const action = rule.action === 'allow' ? 'allow' : 'deny';
    const fromClause = rule.source === '0.0.0.0/0' ? '' : ` from ${rule.source}`;
  ...
  function removeIptablesRule(rule: FirewallRule): void {
    const target = rule.action === 'allow' ? 'ACCEPT' : 'DROP';
    const source = rule.source === '0.0.0.0/0' ? '' : `-s ${rule.source}`;
  ...
  function getBanTimeRemaining(jail: string, ip: string): number {
    try {
  ...
  function scanOpenPorts(): ScanFinding[] {
    const findings: ScanFinding[] = [];
    try {
      // Get listening ports
      ...
      const allowedPorts = new Set(
        (getAllRules.all() as FirewallRuleRow[])
  ...
  function scanCVEs(): ScanFinding[] {
    const findings: ScanFinding[] = [];
    try {
      // Check for available security updates
      const output = execCommand(
        'apt list --upgradable 2>/dev/null | grep -i security || echo ""'
      );
  ...
  function scanSSHConfig(): ScanFinding[] {
    const findings: ScanFinding[] = [];
  ...
  function scanWorldReadableFiles(): ScanFinding[] {
    const findings: ScanFinding[] = [];
    ...
    try {
      const output = execCommand(
        `find /etc /root /home -perm -o=r -type f \\( -name "*.key" -o -name "*.pem" -o -name "id_rsa" -o -name "id_ed25519" -o -name ".env" -o -name "shadow" \\) 2>/dev/null | head -20 || echo ""`
      );
  ...
  function scanDockerSecurity(): ScanFinding[] {
    const findings: ScanFinding[] = [];

    try {
      // Check if Docker daemon is exposed on TCP
      const dockerOutput = execCommand(
      ...
      try {
        ...
        if (containers.trim()) {
          ...
          for (const name of names.slice(0, 10)) { // Check first 10
            try {
              const inspect = execCommand(
                `docker inspect --format "{{.HostConfig.Privileged}}" ${name} 2>/dev/null`
              );
  ...
  function calculateScanScore(findings: ScanFinding[]): number {
  ...
  async function getScanHistory(): Promise<ScanResult[]> {
    const rows = getScanHistoryStmt.all(`-${scanRetentionDays} days`) as SecurityScanRow[];
    return rows.map((row) => ({
      id: row.id,
      timestamp: row.timestamp,
      score: row.score,
      findingCount: row.finding_count,
      findings: JSON.parse(row.findings) as ScanFinding[],
    }));
  }

  // ─── One-Click Hardening ────────────────────────────────────────────────

  async function applyHardening(): Promise<string> {
    const steps: HardeningStep[] = buildHardeningSteps();
    const appliedSteps: HardeningStep[] = [];
    const hardeningId = uuidv4();
    ...
    try {
      ...
      for (let i = appliedSteps.length - 1; i >= 0; i--) {
        try {
          ...
          rollbackErrors.push(
            `Failed to rollback "${appliedSteps[i].name}": ${rollbackError instanceof Error ? rollbackError.message : String(rollbackError)}`
          );
        }
      }
      ...
      throw new Error(
        `Hardening failed at step "${failedStep}": ${errorMsg}.${rollbackInfo}`
      );
    }
  }

  function buildHardeningSteps(): HardeningStep[] {
    const sshConfigPath = '/etc/ssh/sshd_config';
  ...
  function rowToFirewallRule(row: FirewallRuleRow): FirewallRule {
    return {
      id: row.id,
      port: row.port,
      protocol: row.protocol as FirewallProtocol,
      source: row.source,
      action: row.action as FirewallAction,
...
function defaultExecCommand(cmd: string): string {
  return execSync(cmd, { encoding: 'utf-8', timeout: 10_000 });
}

function detectUfw(execCommand: (cmd: string) => string): boolean {
  try {
 ...
 */
function isValidIPOrCIDR(source: string): boolean {
  if (source === '0.0.0.0/0') return true;
...
function cidrsOverlap(cidr1: string, cidr2: string): boolean {
  if (cidr1 === '0.0.0.0/0' || cidr2 === '0.0.0.0/0') return true;
 ...
 */
function ipMatchesAnyAdmin(source: string): boolean {
